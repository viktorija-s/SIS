...
public class PDFCreatorImpl implements IPDFCreatorService {
	...
	private Paragraph createCenteredParagraph(String text, float fontSize) {
		Paragraph paragraph = new Paragraph(text,
				FontFactory.getFont(FontFactory.HELVETICA, fontSize, new Color(8, 20, 64)));
			...
			for (KursaDalibnieki k : dalibnieki) {
				if (k.getSertifikati() != null) {
					...
					Paragraph vardsUnUzvards = new Paragraph(kursaDalibniekaVardsUnUzvards,
							FontFactory.getFont(FontFactory.HELVETICA_BOLD, 23.8f, new Color(52, 79, 179)));
					...
					if (hasFull) {
						...
						document.add(new Paragraph(
								"\nResults the participant has achieved after completing the program " + kursaNosaukums
										+ ":",
								FontFactory.getFont(FontFactory.HELVETICA, 11.8f, new Color(8, 20, 64))));
						...
						document.add(new Paragraph("\nAssessment: " + vertejums + " out of 10",
								FontFactory.getFont(FontFactory.HELVETICA, 16.3f, new Color(8, 20, 64))));

						document.add(new Paragraph("\n\n"));

					} else {

						document.add(createCenteredParagraph("\nhas participated in a non-formal educational program",
								11.8f));
					...
					document.add(new Paragraph(
							...
							FontFactory.getFont(FontFactory.HELVETICA, 10, new Color(8, 20, 64))));
